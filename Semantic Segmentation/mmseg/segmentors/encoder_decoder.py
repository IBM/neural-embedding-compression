...
class EncoderDecoder(BaseSegmentor):
    ...
    def __init__(
        ...
        if freeze_backbone:
            ...
            # self.backbone.eval()
            for param in self.backbone.first_conv.parameters():
                param.requires_grad_(True)
            for param in self.backbone.conv_blocks.parameters():
                param.requires_grad_(True)
            for param in self.backbone.upscaling_blocks.parameters():
                param.requires_grad_(True)
            if self.backbone.with_decoder:
                for param in self.backbone.decoder_blocks[0].parameters():
                    param.requires_grad_(True)
                self.backbone.decoder_embed.requires_grad_(True)
        if neck is not None:
